...
def mean_std_normalization(x):
    ...
    Calculated dynamically per-sample across the Time and Freq axes.
    """
    # Calculate mean and std across Time (axis 1) and Freq (axis 2)
    # keeping dimensions so it broadcasts perfectly across the tensor
    mean = tf.reduce_mean(x, axis=[1, 2], keepdims=True)
    std = tf.math.reduce_std(x, axis=[1, 2], keepdims=True)
    
    # Subtract mean and divide by standard deviation 
    # (adding a tiny epsilon to prevent division by zero during pure silence)
    return (x - mean) / (std + 1e-5)

def rescale(x):
    """
    Dynamically scales an input tensor to exactly [-1, 1] 
    based on the min/max of each sample in the batch.
    """
    # Find the min and max across the Time and Freq axes (axes 1 and 2)
    # We keep dims so it broadcasts correctly across the tensor
    min_val = tf.reduce_min(x, axis=[1, 2], keepdims=True)
    max_val = tf.reduce_max(x, axis=[1, 2], keepdims=True)
    
    # 1. Scale to [0, 1] (add epsilon 1e-5 to prevent division by zero in pure silence)
    x_scaled = (x - min_val) / (max_val - min_val + 1e-5)
    
    # 2. Shift and scale to [-1, 1]
    # return (x_scaled * 2.0) - 1.0
...
def make_model():
    ...
    # 1. Generate leaner spectrogram
    spect = keras.layers.MelSpectrogram(
        sampling_rate=SAMPLE_RATE,
        min_freq=3000,
        max_freq=9000,
        num_mel_bins=224, # Reduced from 224 for massive latency gains
        fft_length=1024,
        power_to_db=True
    )(inp)
    ...
    # 3. Swap magma for physics channels (Mel, Delta, Delta-Delta)
    # This outputs a tensor of shape (Batch, Time, Freq, 3)
    x = keras.layers.Lambda(add_physics_channels)(spect)

    x = keras.layers.Lambda(rescale)(x)
    # print(x.shape)

    # x = keras.layers.Lambda(lambda x: tf.transpose(x, [0, 2, 1, 3]))(x)
    # print(x.shape)

    # 4. Use a low-latency, edge-friendly backbone
    # Note: We keep weights="imagenet" as a starting point, even though our 
    # input is audio physics. The network still benefits from the pre-trained 
    # edge-detection filters in the lower layers.
    cnn = keras.applications.MobileNetV3Small(
        weights="imagenet", 
        include_top=False,
        pooling=None,
        input_shape=(224, 376, 3) # (Time, Freq, Channels)
    )
    x = cnn(x)

    # 5. Simplify the head
    # Swapping AttentiveStatsPool for Global Average Pooling saves compute
    # x = keras.layers.GlobalAveragePooling2D()(x)
    x = AttentiveStatsPool()(x)
    x = keras.layers.BatchNormalization()(x)

    # Pruned dense layers
    x = keras.layers.Dense(128, activation='silu')(x)
