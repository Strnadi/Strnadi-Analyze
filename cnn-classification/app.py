...
async def process(file: UploadFile):
    ...
    for i in range(len(segments)):
        ...
        pred_percents = zip(LABELS, map(lambda x: round(float(x), 2) * 100, prediction.flatten()))

        predictions_response.append(
            {
                "interval": interval,
                "label": map(lambda x: x[0], max(list(pred_percents), key=lambda x: x[1])),
